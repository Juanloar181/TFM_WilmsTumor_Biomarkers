...
Realizamos en primer lugar un breve aálisis que permita ver los tipos de cada una de las mutaciones presentes en el fichero de mutaciones. Posteriormente, extraemos los péptidos neoantigénicos derivados de las mutaciones missense presentes en los datos, generando un fichero fasta con los péptidos mutados y otro con los péptidos wild-type correspondientes.
```{r}
library(readr)
library(dplyr)
...
#Gráfica de barras de tipos de mutaciones
library(ggplot2)
ggplot(mutation_counts, aes(x = reorder(Variant_Classification, -count), y = count)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Tipos de Mutaciones en el Dataset",
       x = "Tipo de Mutación",
...
if (nrow(isoformas) == 0) {
  ...
  pep_mut <- extraer_peptido_proteina(mut_protein, pos_aa, window=8)
    ...
    # Verificar que ambos péptidos tengan exactamente 17 aminoácidos
...
Realizamos un breve análisis en el que vemos la posible repetición de mutaciones para encontrar neoantígenos públicos.
```{r}
#Contar repeticiones de mutaciones
mutacion_counts <- missense_muts %>%
  group_by(HGVSp_Short) %>%
  summarise(count = n()) %>%
  arrange(desc(count))
print(mutacion_counts)
```
